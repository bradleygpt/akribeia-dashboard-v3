...
export default function LoadingOverlay({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    const DTGT = [
      ...
      { x: 306.9, y: 203.8, o: 4 },
      { x: 394.8, y: 471.1, o: 7 },
      { x: 397.4, y: 311.1, o: 13 },
      { x: 399.1, y: 217.9, o: 3 },
      { x: 405.2, y: 423.3, o: 10 },
      { x: 453.2, y: 316.8, o: 11 },
      { x: 463.4, y: 265.6, o: 8 },
      { x: 474.1, y: 222.3, o: 5 },
      { x: 527.4, y: 309.7, o: 14 },
      { x: 554.1, y: 226.0, o: 9 },
      { x: 597.4, y: 307.2, o: 16 },
      { x: 620.7, y: 366.2, o: 18 },
      { x: 633.3, y: 234.6, o: 12 },
      { x: 665.5, y: 435.5, o: 20 },
      { x: 667.1, y: 280.3, o: 19 },
      { x: 671.1, y: 465.5, o: 21 },
      { x: 686.0, y: 243.7, o: 15 },
      { x: 705.7, y: 497.4, o: 23 },
      { x: 722.3, y: 461.5, o: 22 },
      { x: 738.5, y: 258.4, o: 17 },
    ];
    const DLINES = [
      [1, 4],
      [4, 7],
    ...
    function quantGradient(c: CanvasRenderingContext2D) {
      ...
      g.addColorStop(0, "#ee7d6b");
      g.addColorStop(0.45, "#e55b75");
      g.addColorStop(0.75, "#de4a8a");
      g.addColorStop(1, "#ca49a3");
      return g;
    }
    const TO = [
        [352, 438],
    ...
    function drawLogoImage(c: CanvasRenderingContext2D, alpha: number) {
      c.save();
      c.globalAlpha = alpha;
      c.drawImage(LIMG, LP.x, LP.y, LP.w, LP.h);
      c.restore();
    }
    ...
    function pushLine(
      ...
      const d = Math.hypot(x1 - x0, y1 - y0),
        n = Math.max(2, Math.ceil(d / step));
      for (let i = 1; i <= n; i++)
        ROUTE.push({ x: lerp(x0, x1, i / n), y: lerp(y0, y1, i / n), pen, tag });
    }
    function pushCurve(
      x0: number,
      y0: number,
      ...
      const n = Math.max(
        6,
        Math.ceil((Math.hypot(cx - x0, cy - y0) + Math.hypot(x1 - cx, y1 - cy)) / step),
      );
      for (let i = 1; i <= n; i++) {
        const t = i / n,
          a = 1 - t;
        ROUTE.push({
          x: a * a * x0 + 2 * a * t * cx + t * t * x1,
          y: a * a * y0 + 2 * a * t * cy + t * t * y1,
          pen,
        });
      }
    }
    ...
    function buildRoute() {
      ...
      if (VARIANT === "ai") {
        ...
        for (let r = 0; r <= rows; r++) {
          ...
          if (r < rows)
            pushLine(ltr ? X1 : X0, y, ltr ? X1 : X0, lerp(Y0, Y1, (r + 1) / rows), 1, 5, "sweep");
        }
        ...
        const sx = q.cx + q.r * Math.cos(q.a0),
          sy = q.cy + q.r * Math.sin(q.a0);
        pushCurve(q.dotX, q.dotY, (q.dotX + sx) / 2 - 30, (q.dotY + sy) / 2, sx, sy, 0);
        pushArc(q.cx, q.cy, q.r, q.a0, q.a1, 1, 4, "ring");
        const ex = q.cx + q.r * Math.cos(q.a1),
          ey = q.cy + q.r * Math.sin(q.a1);
    ...
    function sample(drawFn: (c: CanvasRenderingContext2D) => void, step: number) {
      ...
      const d = c.getImageData(0, 0, DW, DH).data,
        pts: { x: number; y: number; col: number[] }[] = [];
    ...
    function initWorld() {
      ...
      const b = vb();
      for (let i = 0; i < N; i++) {
        const bt = bearPts[i % bearPts.length],
          lt = logoPts[i % logoPts.length];
        P.push({
          hx: rand(b.x0, b.x1),
          hy: rand(b.y0, b.y1),
          x: 0,
          ...
          struck: -1,
          boomed: false,
          boomA: rand(0, Math.PI * 2),
          boomS: rand(60, 320) * (Math.random() < 0.12 ? 2.1 : 1),
        });
        P[i].x = P[i].hx;
        P[i].y = P[i].hy;
      }
      DSTARS = DTGT.map((t, k) => {
        ...
        return {
          ...
          ly: lt.y,
          lcol: lt.col,
          start: 0.06 + (t.o !== undefined ? t.o : k) * (0.74 / DTGT.length),
          landedAt: -1,
          conv: 0,
          struck: -1,
          boomed: false,
    ...
    const beat = (el: number) => {
      const T = (el / 1150) % 1,
        g = (m: number, s: number) => Math.exp(-((T - m) * (T - m)) / (2 * s * s));
    ...
    function frame(now: number) {
      ...
      const el = now - t0,
        ...
        ph = PHASES[pi].name;
      if (pi !== lastPhase) {
        caption.textContent = PHASES[pi].cap;
        lastPhase = pi;
      }
      ...
      vg.addColorStop(0, "rgba(8,13,22,0)");
      ...
      if (bearGlow > 0) {
        ...
        bg.addColorStop(0.5, "rgba(148,110,248,.32)");
        bg.addColorStop(1, "rgba(96,142,255,.26)");
        ctx.globalAlpha = bearGlow;
        ctx.fillStyle = bg;
        bearPath(ctx);
        ctx.fill();
        ctx.strokeStyle = "rgba(198,172,255,.6)";
        ctx.lineWidth = 1.6;
        ctx.shadowColor = "rgba(172,140,255,.9)";
        ctx.shadowBlur = 26 * bearGlow;
        ctx.stroke();
        ctx.restore();
      }
      ...
      if (pi >= 1 && pi <= 2) {
        for (const s of DSTARS) {
          ...
          const e = easeIO(f);
          s.x = lerp(s.hx, s.tx, e);
          s.y = lerp(s.hy, s.ty, e);
          if (f >= 1) {
            s.x = s.tx + Math.sin(tsec * 2 + s.start * 40) * 0.8;
            s.y = s.ty + Math.cos(tsec * 1.7 + s.start * 40) * 0.8;
          }
        }
        ctx.save();
        ctx.strokeStyle = "rgba(236,240,255,.85)";
        ctx.lineWidth = 1.4;
        ...
        for (const [a, c2] of DLINES) {
          ...
          ctx.beginPath();
          ctx.moveTo(A.x, A.y);
          ctx.lineTo(lerp(A.x, B.x, lp), lerp(A.y, B.y, lp));
          ctx.stroke();
        }
        for (const s of DSTARS) {
          ...
          if (inFlight) {
            ...
            if (since < 1) {
              ...
              ctx.lineWidth = 1.5;
              ctx.beginPath();
              ctx.arc(s.tx, s.ty, 4 + since * 22, 0, 7);
              ctx.stroke();
              ctx.globalAlpha = 1;
            }
          }
      ...
      for (const q of P) {
        if (ph === "starfield") {
          q.x = q.hx + Math.sin(tsec * 0.3 + q.tw) * 2;
          q.y = q.hy + Math.cos(tsec * 0.25 + q.tw) * 2;
        } else if (ph === "converge") {
          const st = 0.18 + q.oi * 0.62,
            f = clamp((p - st) / 0.16, 0, 1),
            e = easeO(f);
          const x = lerp(q.hx, q.bx, e),
            y = lerp(q.hy, q.by, e);
          const sq = 1 - 0.045 * b * e;
          q.x = BEAR_CX + (x - BEAR_CX) * sq;
          q.y = BEAR_CY + (y - BEAR_CY) * sq;
        } else if (ph === "bear") {
          const sq = 1 - 0.02 * b;
          ...
          if (!q.boomed) {
            q.boomed = true;
            q.vx = Math.cos(q.boomA) * q.boomS;
            q.vy = Math.sin(q.boomA) * q.boomS;
          }
          q.x += q.vx * dt;
          q.y += q.vy * dt;
          q.vx *= 0.985;
      ...
      if (pi >= 3) {
        for (const s of DSTARS) {
          if (ph === "bigbang") {
            if (!s.boomed) {
              s.boomed = true;
              s.vx = Math.cos(s.boomA) * s.boomS;
              s.vy = Math.sin(s.boomA) * s.boomS;
            }
            s.x += s.vx * dt;
            s.y += s.vy * dt;
            s.vx *= 0.985;
      ...
      if (ph === "bigbang" && p < 0.5) {
        ctx.setTransform(DPR, 0, 0, DPR, 0, 0);
        ctx.fillStyle = `rgba(255,255,255,${(1 - p * 2) * 0.9})`;
        ctx.fillRect(0, 0, W, H);
        ctx.setTransform(S * DPR, 0, 0, S * DPR, OX * DPR, OY * DPR);
      }
        ...
        if (ph === "draw") {
          ...
          while (eventCursor < EVENTS.length && idx >= EVENTS[eventCursor].idx) {
            if (EVENTS[eventCursor].type === "dot") {
              ...
              burst(QA.dotX, QA.dotY, "#ca49a3", 8);
              burst(QA.dotX, QA.dotY, "#ffffff", 5);
            }
            eventCursor++;
          }
          const R2 = 36 * 36;
          ...
          for (let i = 1; i < TRAIL.length; i++) {
            ...
            ctx.strokeStyle = `rgba(220,232,244,${f * 0.65})`;
            ctx.lineWidth = 0.5 + f * 3.4;
            ctx.shadowColor = "rgba(190,215,240,.8)";
            ctx.shadowBlur = 8 * f;
            ctx.beginPath();
          ...
          dg.addColorStop(0.35, "#d7e9f2");
          dg.addColorStop(0.75, "#61798c");
          dg.addColorStop(1, "#1e2d3a");
          ctx.fillStyle = dg;
          ctx.beginPath();
          ctx.moveTo(0, -40);
          ...
          ctx.beginPath();
          ctx.ellipse(-5, -2, 2.4, 7, -0.35, 0, 7);
          ctx.fill();
          ctx.restore();
        } else {
          TRAIL.length = 0;
          ...
          logoCrisp(ctx, la);
          if (p > 0.3 && !wordmark.classList.contains("on")) wordmark.classList.add("on");
          if (p >= 1 && !done) {
            done = true;
    ...
    LIMG.src =
      VARIANT === "ai" ? "/landing/akribeia-mark-ai.png" : "/landing/akribeia-mark-quant.png";
  ...
  return (
    ...
    >
      ...
      <div ref={captionRef} className="akl-caption">
        INITIALIZING
      </div>
      <div className="akl-bar">
        <i ref={barfillRef as React.RefObject<HTMLElement>} />
      </div>
      {/* SKIP is present from the first frame, not only at the end */}
      <button className="akl-skip" onClick={() => finish.current(SKIP_FADE_MS)}>
        skip →
      </button>
    </div>
  );
