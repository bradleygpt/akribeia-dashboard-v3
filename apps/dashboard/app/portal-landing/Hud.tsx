...
// HUD for the tri-star portal landing — ported from the approved Vercel landing
// (quant-dashboard-pro-v2 src/landing/Hud.tsx). Layout, palette and controls are
// unchanged; the status strip now reads V3's own governed state: evidence
// freshness (fail-closed), AI mode, and schema version — no pro-v2 metrics.
...
export default function Hud(props: Props) {
  ...
  const scrim = daylight
    ? "rounded-md bg-[#070b12]/55 px-3 py-2.5 backdrop-blur-[2px] ring-1 ring-white/10"
    : "";

  // Evidence tier display — fail-closed: an unhealthy endpoint reads FAIL-CLOSED
  // regardless of the source-observation age.
  const tier = s.evidence.healthy ? s.evidence.tier : "failclosed";
  const evidenceValue =
    tier === "fresh"
      ? "✓ FRESH"
      : tier === "amber"
  ...
  return (
    <div className="pointer-events-none absolute inset-0 select-none font-mono text-[#C3CAD7]">
      ...
      {/* ---------- status strip (top-right): evidence/mode/schema ---------- */}
      <div className={`absolute right-6 top-5 text-right ${scrim}`}>
        <div className="flex justify-end gap-6 tabular-nums">
          <Stat label="EVIDENCE" value={evidenceValue} tone={evidenceTone} />
          <Stat
            label="MODE"
            value={s.externalModelConfigured ? "LLM LIVE" : "DETERMINISTIC"}
            tone={s.externalModelConfigured ? "#00C805" : "#FF9800"}
          />
          <Stat label="SCHEMA" value={s.schemaVersion ?? "—"} tone="#9CA7BB" />
        </div>
      ...
      <div className={`pointer-events-auto absolute bottom-6 right-6 w-[260px] ${scrim}`}>
        ...
        <div className="mt-4 flex items-center justify-between">
          ...
          <button onClick={resetChaos} className={`${micro} transition hover:text-[#C3CAD7]`}>
            reset → {computeChaos(s).toFixed(2)}
          </button>
        </div>
        <input
          type="range"
        ...
        >
          <div className="flex items-center gap-2">
            <span
              ...
              style={{ background: hover.def.accent }}
            />
            <span className="text-[12px] tracking-[0.12em] text-[#DCE3EE]">{hover.def.name}</span>
          </div>
          <div className="mt-1 text-[10px] tracking-[0.06em] text-[#9CA7BB] tabular-nums">
            {hover.def.status}
          </div>
        </div>
      )}
        ...
        >
          <div className="flex items-center gap-2">
            <span
              ...
              style={{ background: sunHover.def.color, boxShadow: `0 0 8px ${sunHover.def.color}` }}
            />
            <span className="text-[12px] tracking-[0.16em] text-[#DCE3EE]">
              {sunHover.def.name}
            </span>
            <span className={micro}>engine</span>
          </div>
          <div className="mt-1 text-[10px] tracking-[0.06em] text-[#9CA7BB]">
            {sunHover.def.role}
          </div>
          <div className="mt-0.5 text-[10px] tracking-[0.06em] text-[#7C879B] tabular-nums">
            {sunHover.def.status}
          </div>
        </div>
      )}
