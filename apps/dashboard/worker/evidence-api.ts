...
/**
 * Loads the served baked-fundamentals artifact (generated by
 * scripts/build-ai-fundamentals.node.mjs) and narrows it server-side to one
 * ticker. Returns null when the artifact or the ticker entry is unavailable so
 * the research kind fails closed instead of prompting without grounding.
 */
async function loadFundamentalsSecurity(
  env: EvidenceApiEnv,
  request: Request,
  ticker: string,
): Promise<AiFundamentalsSecurity | null> {
  const response = await fetchJsonAsset(env, request, "/data/ai-fundamentals.json");
  ...
  try {
    const artifact = (await response.json()) as {
      securities?: Record<string, AiFundamentalsSecurity>;
    };

    return artifact.securities?.[ticker] ?? null;
  } catch {
    return null;
  }
}

function figure(value: number | undefined, digits = 2): string {
 ...
 */
function researchFactBlock(
  security: PublishedScoredSecurity,
  fundamentals: AiFundamentalsSecurity,
): PortfolioFactBlock {
  const contributions = security.contributions
    .map((contribution) =>
      contribution.status === "available"
    ...
    .join("; ");
  const grades = Object.entries(fundamentals.grades ?? {})
    .map(([pillar, grade]) => `${pillar} ${grade}`)
    .join(", ");
  ...
  const facts = [
    ...
    `Factor coverage: ${(security.coverage * 100).toFixed(0)}%`,
    `Factor contributions: ${contributions}`,
    grades.length > 0 ? `Pillar grades — ${grades}` : "",
    typeof fundamentals.fv === "number"
      ? `Fair value $${fundamentals.fv.toFixed(2)}: premium ${figure(fundamentals.fvPremium, 1)}% — verdict ${fundamentals.fvVerdict ?? "?"}.`
      : "",
    typeof fundamentals.qbp === "number"
  ...
  return {
    facts,
    citations: ["scores.json:security", `ai-fundamentals.json:${security.ticker}`],
  };
}

/**
 * Ported from the V2 AI endpoint's research kind: a four-paragraph analyst
 * research note grounded ONLY in the supplied published figures, with V3's
 * no-advice constraint layered on top.
 */
function researchPrompt(ticker: string, facts: string): string {
  return [
    "You are a senior equity analyst for Akribeia, a quantitative research",
    `preview. Write a 4-paragraph research note on ${ticker}.`,
    "",
    "DATA (verified published artifacts — the ONLY figures you may cite; do not",
    "recompute prices or invent numbers, price targets, or market data):",
    facts,
    "",
    "Write 4 paragraphs (no headers, no bullet points):",
    "1) Thesis — what is working, citing the strong pillar grades and specific",
    "metrics above (margins, growth, momentum).",
    "2) Risks — honest and specific to this name, citing the weak grades and",
    "metrics.",
...
async function handleAiAssist(
  ...
  if (input.kind === "research") {
    const security = findSecurity(evidence, input.ticker);
    ...
    const fundamentals = await loadFundamentalsSecurity(env, request, security.ticker);
